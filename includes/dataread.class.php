...
class DataRead {
    ...
    function userHasProfileInTargetSeason($season_id, $user_id) {
    ...
    function userSearch($season_id, $current_user_id, $search_term) {
        ...
        if (db_num_rows($query)) {
            while ($row = db_fetch_array($query)) {
                ...
                if (empty($searchuser->user_reg_type)) {
                    $searchuser->user_reg_type = 0;
                }
    ...
    function getIndexStatus($user_id, $season_id, $reg_type) {
        ...
        if ($this->userHasProfileInTargetSeason($season_id, $user_id)) {
