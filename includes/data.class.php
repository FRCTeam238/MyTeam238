...
class Data extends DataRead {
    ...
    function doCreateAccount($email, $password, $emailKey, $access_code_used) {
        ...
        if (!isset($access_code_used) || $access_code_used == NULL) {
            $access_code_used = 0;
        }
    ...
    function doAccountLogin($season_id, $email, $password) {
        ...
        if ($passMatch && db_num_rows(db_query($sql1))) { //pass good, and user row exists
            ...
            if ($this->userHasProfileInTargetSeason($season_id, $login_holding['user_id'])) {
    ...
    function joinSeason($user_id, $season_id, $pref_name, $seasonrole) {
        ...
        if (!$this->userHasProfileInTargetSeason($season_id, $user_id) && $seasonrole != "Sponsor") {
    ...
    function doAttemptCopyValuesFromPreviousSeason($user_id, $season_id_current) {
        //We're going to attempt to help them by copying the values for
        //profile (like address)
        try {
            $season_id_copyFrom = ($season_id_current - 1);
            require_once(CLASSES_DIR . 'season_profile.php');
            $DataRead = new DataRead;

            //PROFILE DATA
            $season_profile_previous = $DataRead->getCurrentSeasonProfile($user_id, $season_id_copyFrom);
            //move applicable values into the user
            //check for null values so we don't make them empty string
            $cell_use = empty($season_profile_previous->cell_phone) ? null : $season_profile_previous->cell_phone;
            $gender_use = empty($season_profile_previous->gender) ? null : $season_profile_previous->gender;
            $shirt_use = empty($season_profile_previous->shirt_size) ? null : $season_profile_previous->shirt_size;
            $bio_use = empty($season_profile_previous->biography) ? null : $season_profile_previous->biography;
            $addr1_use = empty($season_profile_previous->address_1) ? null : $season_profile_previous->address_1;
            $addr2_use = empty($season_profile_previous->address_2) ? null : $season_profile_previous->address_2;
            $city_use = empty($season_profile_previous->address_city) ? null : $season_profile_previous->address_city;
            $state_use = empty($season_profile_previous->address_state) ? null : $season_profile_previous->address_state;
            $zip_use = empty($season_profile_previous->address_zip) ? null : $season_profile_previous->address_zip;

            $this->doUpdateSeasonProfile($user_id, $season_id_current, $cell_use, $gender_use, $shirt_use, $bio_use, $addr1_use, $addr2_use, $city_use, $state_use, $zip_use);

            //EMERGENCY CONTACT
            if ($season_profile_previous->emergency_contact_id != null) {
                $this->doAddEmergencyContact_ByContactId($user_id, $season_id_current, $season_profile_previous->emergency_contact_id);
            } else if ($season_profile_previous->emergency_contact_user_id != null) {
                $this->doAddEmergencyContact_ByUserId($user_id, $season_id_current, $season_profile_previous->emergency_contact_id);
            }
    ...
    function doAddEmergencyContact_ByUserId($user_id, $season_id, $emer_contact_user_id) {
        $sql1 = "UPDATE " . TABLE_PROFILE . " "
                . "SET emergency_contact_user_id = " . db_input($emer_contact_user_id) . ", emergency_contact_id = NULL "
                . "WHERE user_id = " . db_input($user_id) . " "
                . "AND season_id = " . db_input($season_id) . ";";
        return db_query($sql1);
    }

    function doAddEmergencyContact_ByContactId($user_id, $season_id, $emer_contact_id) {
        $sql1 = "UPDATE " . TABLE_PROFILE . " "
                . "SET emergency_contact_id = " . db_input($emer_contact_id) . ", emergency_contact_user_id = NULL "
                . "WHERE user_id = " . db_input($user_id) . " "
                . "AND season_id = " . db_input($season_id) . ";";
        return db_query($sql1);
    }
    ...
    function doAddEmergencyContact_Manual($user_id, $season_id, $fname, $lname, $relation, $phone) {
        ...
        if ($insert_id > 0 && $insert_id != NULL) {
            return $this->doAddEmergencyContact_ByContactId($user_id, $season_id, $insert_id);
        } else {
            return 0;
        }
    }
