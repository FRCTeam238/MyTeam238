...
if ($_POST) {//incoming join attempt
    if ($_POST['seasonrole'] != "Select...") {
        ...
        if ($result) {
            ...
            //If desired, also attempt to back-fill their previous year data
            if($_POST['copyValues']) {
                $result2 = $Data->doAttemptCopyValuesFromPreviousSeason($_SESSION['_user']['id'], $_POST['seasonid']);
                if ($result2) {
                    $Data->doLog(1040, $_SESSION['_user']['id'], $_SERVER['REQUEST_URI'], 'Copied previous season profile data');
                } else {
                    $Data->doLog(1019, $_SESSION['_user']['id'], $_SERVER['REQUEST_URI'], 'Issue when attempting to copy previous profile data');
                }
            }
    ...
    if ($Data->userHasProfileInTargetSeason($_SESSION['current_season_id'], $_SESSION['_user']['id'])) {
        ...
        //Check if user had a profile in the previous season, if so we may be able to copy values for them
        $had_profile_last_season = $Data->userHasProfileInTargetSeason(($_SESSION['current_season_id'] - 1), $_SESSION['_user']['id']);
    }
}
        ...
        <div class="panel panel-primary">
            ...
            <div class="panel-body">
                <div class="form-group has-feedback">
                    <label class="control-label" for="pfname">Preferred First Name</label><br />
                    <em>This field is optional. If you don't specify a preferred name, we'll simply use your legal first name as your preferred.</em>
                ...
                <div class="form-group has-feedback">
                    <label class="control-label" for="copyValues">Copy <?php echo ($current_season->season_year - 1) ?> Values</label><br />
                    <em>This field is optional. It looks like you had a profile last year, welcome back! If you'd like, we can attempt to copy 
                        some of the values you provided last year into your profile for this season. Check the box to opt-in.</em><br />
                    <input type="checkbox" name="copyValues" class="form-control" id="copyValues" autocomplete="off" value="true">                
                </div>
